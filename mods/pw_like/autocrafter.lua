...
local function may_configure(pos, player)
...
minetest.register_node("pw_like:autocrafter", {
	...
	on_receive_fields = function(pos, formname, fields, sender)
		if not may_configure(pos, sender) then return end
		local meta = minetest.get_meta(pos)
		if fields.on then
			update_meta(meta, false)
			minetest.get_node_timer(pos):stop()
	...
	allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		if not may_configure(pos, player) then return 0 end
		upgrade_autocrafter(pos)
	...
	allow_metadata_inventory_take = function(pos, listname, index, stack, player)
		if not may_configure(pos, player) then
